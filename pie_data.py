...
class PIE(object):
    ...
    def _get_speed(self, box, next_box):

        center = [(box[0] + box[2]) / 2, (box[1] + box[3]) / 2]
        next_center = [(next_box[0] + next_box[2]) / 2, (next_box[1] + next_box[3]) / 2]
        x = math.pow((next_center[0] - center[0]), 2)
        y = math.pow((next_center[1] - center[1]), 2)
        speed = math.sqrt(x + y)

        return speed

    def _get_distance(self, box):

        veh = [960, 1080]
        center = [(box[0] + box[2]) / 2, (box[1] + box[3]) / 2]
        dis = (math.sqrt(math.pow((center[0] - veh[0]), 2) + math.pow((center[1] - veh[1]), 2)))/100

        return dis

    def _get_gps_distance(self, lng1, lat1, lng2, lat2):
        ...
        distance = round(distance / 1000, 9)

        return distance

    def _get_ped_coord(self, start_box, box):

        start_center = [(start_box[0] + start_box[2]) / 2, (start_box[1] + start_box[3]) / 2]
        center = [(box[0] + box[2]) / 2, (box[1] + box[3]) / 2]
        x = math.pow((center[0] - start_center[0]), 2)
        y = math.pow((center[1] - start_center[1]), 2)
        ped_coor = math.sqrt(x + y)
    ...
    def _get_trajectories(self, image_set, annotations, **params):
        ...
        image_seq, pids_seq = [], []
        box_seq, ped_coord_seq, center_seq, distance_seq, speed_seq, occ_seq = [], [], [], [], [], []
        intent_seq, se_seq, un_seq = [], [], []
        ...
        for sid in set_ids:
            for vid in sorted(annotations[sid]):
                ...
                for pid in sorted(pid_annots):
                    ...
                    se_seq.append([[0] for b in boxes][::seq_stride])
                    un_seq.append([[0] for b in boxes][::seq_stride])
                    ped_coord_seq.append(
                        [[self._get_ped_coord(boxes[0], boxes[i])] for i in range(len(boxes))][::seq_stride])

                    center_seq.append([self._get_center(b) for b in boxes][::seq_stride])

                    distance_seq.append([[self._get_distance(b)] for b in boxes][::seq_stride])
                    ...
                    speed_seq.append(
                        [[self._get_speed(boxes[i], boxes[i + 1])] for i in range(len(boxes)) if i + 1 < len(boxes)][
                        ::seq_stride])
                    occ_seq.append(occlusions[::seq_stride])

                    gpsc_seq.append([[self._get_gps_distance(vid_annots[0]['longitude'], vid_annots[0]['latitude'],
                                                             vid_annots[i]['longitude'], vid_annots[i]['latitude'])] for
                                     i in frame_ids][::seq_stride])
                    ...
                    head_ang_seq.append([[vid_annots[i]['heading_angle'] / 360] for i in frame_ids][::seq_stride])
        ...
        return {'image': image_seq,
                'pid': pids_seq,
                'bbox': box_seq,
                'ped_coord': ped_coord_seq,
                'speed_estimation': se_seq,
                'uncertainty': un_seq,
                'center': center_seq,
        ...
        for sid in set_ids:
            for vid in sorted(annotations[sid]):
                ...
                for pid in sorted(pid_annots):
                    ...
                    gpsc_seq.append([[self._get_gps_distance(vid_annots[0]['longitude'], vid_annots[0]['latitude'],
                                                             vid_annots[i]['longitude'], vid_annots[i]['latitude'])]
                                     for i in frame_ids][::seq_stride])
                    obds_seq.append([[vid_annots[i]['OBD_speed']] for i in frame_ids][::seq_stride])
                    gpss_seq.append([[vid_annots[i]['GPS_speed']] for i in frame_ids][::seq_stride])
                    head_ang_seq.append([[vid_annots[i]['heading_angle'] / 360] for i in frame_ids][::seq_stride])
...
if __name__ == '__main__':
    imdb = PIE(data_path='data/pie_dataset')
    data_opts = {'fstride': 1,
                 'sample_type': 'all',
                 'height_rng': [0, float('inf')],
                 'squarify_ratio': 0,
                 'data_split_type': 'default',  # kfold, random, default
                 'seq_type': 'trajectory',  # crossing
                 'min_track_size': 61,
                 'random_params': {'ratios': None,
                                   'val_data': True,
                                   'regen_data': True},
                 'kfold_params': {'num_folds': 5, 'fold': 1}}
    dataset = imdb.generate_data_trajectory_sequence('val', **data_opts)
    print('distance:', np.max(np.array(dataset['distance'])))
    print('speed:', np.max(np.array(dataset['speed'])))
    print('obd_speed:', np.max(np.array(dataset['obd_speed'])))
    print('heading_angle:', np.max(np.array(dataset['heading_angle'])))
    print('gps_coord:', np.max(np.array(dataset['gps_coord'])))
    print('un:', np.max(np.array(dataset['uncertainty'])))


    # data = imdb.balance_samples_count(dataset, label_type='intention_binary')
