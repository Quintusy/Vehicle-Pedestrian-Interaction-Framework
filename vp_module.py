...
class VP_Module(object):
    ...
    def log_configs(self, config_path, batch_size, epochs, lr, loss, learning_scheduler, opts):
    ...
    def loss_fuction(self, truth, pred):
        ...
        for i in range(self._prediction_size):
            loss1 = tf.exp(-tf.clip_by_value(pred[:, :, self._prediction_size + i], 0, 5)) * tf.square(
                truth[:, :, i] - pred[:, :, i])
    ...
    def train(self, num, data_train, data_val,
        ...
        print("Number of samples:\n Train: %d \n Val: %d \n"
              % (data_train['enc_input'].shape[0], data_val['enc_input'].shape[0]))

        self._observe_length = data_train['enc_input'].shape[1]
        self._predict_length = data_train['pred_target'].shape[1]
        self._encoder_feature_size = data_train['enc_input'].shape[3]
        self._decoder_feature_size = data_train['dec_input'].shape[2]
        self._prediction_size = data_train['pred_target'].shape[2]
        ...
        with open(opts_path, 'wb') as fid:
            pickle.dump(data_train['model_opts'], fid,
        ...
        self.log_configs(config_path, batch_size, epochs,
                         ...
                         data_train['model_opts'])

        if exists(model_path) and getsize(model_path):
            vp_model = load_model(model_path, custom_objects={'loss_fuction': loss})
        else:
            vp_model = self.vp_model()
            # vp_model.load_weights(model_path)
        ...
        history = vp_model.fit(x=train_data[0], y=train_data[1],
    ...
    def test(self, num, data_test, mc_times=100, **model_opts):
        ...
        test_obs_data = ([data_test['enc_input'][:, 0],
                          data_test['enc_input'][:, 1],
                          data_test['dec_input']])
        test_target_data = data_test['pred_target']
        ...
        np.square(test_target_data[:, :, :4] - testY)
